...
public class ModelBreedableParrot extends ModelBase
{
    ModelRenderer body;
    ModelRenderer tail;
    ...
    private ModelBreedableParrot.State state = ModelBreedableParrot.State.STANDING;
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn)
    {
        ...
        if (this.state != ModelBreedableParrot.State.FLYING)
        {
            if (this.state == ModelBreedableParrot.State.SITTING)
            {
                return;
            }

            if (this.state == ModelBreedableParrot.State.PARTY)
            {
    ...
    public void setLivingAnimations(EntityLivingBase entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTickTime)
    {
        ...
        if (entitylivingbaseIn instanceof EntityParrot)
        {
            ...
            if (entityparrot.isPartying())
            {
                ...
                this.state = ModelBreedableParrot.State.PARTY;
                return;
            }
            ...
            if (entityparrot.isSitting())
            {
                ...
                this.state = ModelBreedableParrot.State.SITTING;
            }
            else if (entityparrot.isFlying())
            {
                this.legLeft.rotateAngleX += ((float)Math.PI * 2F / 9F);
                this.legRight.rotateAngleX += ((float)Math.PI * 2F / 9F);
                this.state = ModelBreedableParrot.State.FLYING;
            }
            else
            {
                this.state = ModelBreedableParrot.State.STANDING;
            }
